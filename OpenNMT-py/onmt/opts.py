...
def model_opts(parser):
    ...
    group.add('--decoder_type', '-decoder_type', type=str, default='rnn',
              choices=['rnn', 'transformer', 'cnn', 'transformerDPPPrevl', 'transformerDPPPrevst',
                       'transformerDPPPrevlP', 'transformerDPPPrevlSS', 'transformerDPPPrevlSupr',
                       'transformerDPPPrevlEQK'],
              help="Type of decoder layer to use. Non-RNN layers "
                   "are experimental. Options are "
                   "[rnn|transformer|cnn].")

    group.add_argument('--dpp_attention', '-dpp_attention', action="store_true",
                       help='If present, adds DPP step w.r.t. previous context over the attention.'
                            'Used by BRNN Input feed decoder.')
    group.add_argument('--dpp_rescaled', '-dpp_rescaled', action="store_true",
                       help='if True, re-scale attention weights after dpp-weights multiplication.')
    group.add_argument('--dpp_ctx', '-dpp_ctx', action="store_true",
                       help='if True, used by brnn decoder type to do dpp attention w.r.t the previous source context.')
    group.add_argument('--single_dpp_head', '-single_dpp_head', action="store_true",
                       help='if True, use a single head for decoder-encoder attention.')
    ...
    group.add('--lambda_coverage', '-lambda_coverage', type=float, default=0.0,
     ...
     group.add('--coverage_attn_feed', '-coverage_attn_feed', action="store_true",
              help='Train a coverage attention layer. Feed previous attention weights ')
...
def translate_opts(parser):
    ...
    group.add('--attn_debug', '-attn_debug', action="store_true",
      ...
      group.add('--dectemp', '-dectemp', type=float, default=1.,
              help="Cross attention temperature on decoding.")
