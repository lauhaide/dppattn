...
from onmt.modules import context_gate_factory, GlobalAttention, GlobalAttentionDPP
...
class RNNDecoderBase(DecoderBase):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
                 ...
                 coverage_attn=False, coverage_attn_feed=False, context_gate=None,
                 copy_attn=False, dropout=0.0, embeddings=None,
                 reuse_copy_attn=False, copy_attn_type="general",
                 dpp_attn=False, dpp_rescaled=False, dpp_ctx=False):
        ...
        # dpp stuff
        if dpp_attn:
            print("*    DPP attention with dpp_rescaled={} and dpp_ctx={}".format(dpp_rescaled, dpp_ctx))
        self.dpp_attn = dpp_attn
        self.dpp_ctx = dpp_ctx
        ...
        self.coverage_attn_feed = coverage_attn_feed
        ...
        if not self.attentional:
            ...
            if dpp_attn:  ## dpp attention
                self.attn = GlobalAttentionDPP(
    ...
    def init_state(self, src, memory_bank, encoder_final):
        ...
        self.state["prev_context"] = \
            self.state["hidden"][0].data.new(*h_size).fill_(0).unsqueeze(0)
    ...
    def map_state(self, fn):
        ...
        self.state["prev_context"] = fn(self.state["prev_context"], 1)

    def detach_state(self):
        self.state["hidden"] = tuple(h.detach() for h in self.state["hidden"])
        self.state["input_feed"] = self.state["input_feed"].detach()
        self.state["prev_context"] = self.state["prev_context"].detach()
    ...
    def forward(self, tgt, memory_bank, memory_lengths=None, step=None):
        ...
        dec_state, dec_outs, attns, ctx = self._run_forward_pass(
            tgt, memory_bank, memory_lengths=memory_lengths, step=None)
        ...
        self.state["prev_context"] = ctx.unsqueeze(0)
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, memory_lengths=None, step=None):
        """
        ...
        ctx = self.state["prev_context"].squeeze(0)
        ...
        for i, emb_t in enumerate(emb.split(1)):
            ...
            if self.attentional:
                ...
                if self.dpp_attn:
                    ctx_step = step if step is not None else i  # take previous step, either train or inference
                    if ctx_step==0:
                        ctx_step=1 # default very first step also divides the context by 1

                    decoder_output, p_attn, c = self.attn(
                        rnn_output,
                        memory_bank.transpose(0, 1),
                        memory_lengths=memory_lengths,
                        prev_context=(ctx / ctx_step),
                        in_s_set=coverage)

                    if (i == 0 or (step is not None and step == 0)):
                        ctx = (c if self.dpp_ctx else decoder_output)
                    else:
                        ctx = ctx + (c if self.dpp_ctx else decoder_output)
                elif self.coverage_attn_feed:
                    decoder_output, p_attn = self.attn(
                        rnn_output,
                        memory_bank.transpose(0, 1),
                        memory_lengths=memory_lengths,
                        coverage=coverage) # modif. LP
                else:
                    decoder_output, p_attn = self.attn(
                        rnn_output,
                        memory_bank.transpose(0, 1),
        ...
        return dec_state, dec_outs, attns, ctx
