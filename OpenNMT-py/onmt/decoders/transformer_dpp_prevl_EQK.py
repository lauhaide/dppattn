...
from onmt.modules import MultiHeadedAttention, AverageAttention, MultiHeadedAttentionDPPExpQK
from onmt.modules.position_ffn import PositionwiseFeedForward
from onmt.utils.misc import sequence_mask

from onmt.decoders.transformer import TransformerDecoder

class TransformerDecoderLayerDPPPrevlEQK(nn.Module):
    """
    Args:
    ...
    def __init__(self, d_model, heads, d_ff, dropout, attention_dropout,
        ...
        super(TransformerDecoderLayerDPPPrevlEQK, self).__init__()
        ...
        self.context_attn = MultiHeadedAttentionDPPExpQK(
...
class TransformerDecoderDPPPrevlEQK(TransformerDecoder):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff,
        ...
        super(TransformerDecoderDPPPrevlEQK, self).__init__(num_layers, d_model, heads, d_ff,
                 copy_attn, self_attn_type, dropout, attention_dropout,
        ...
        self.transformer_layers = nn.ModuleList(
            [TransformerDecoderLayerDPPPrevlEQK(d_model, heads, d_ff, dropout,
    ...
    def forward(self, tgt, memory_bank, step=None, prev_key_contexts=None, dectemp=1.0, **kwargs):
        ...
        for i, layer in enumerate(self.transformer_layers):
            ...
            if i > 0 and step is None: #training
                ...
                dstidx = torch.arange(1, key_contexts.size()[1]).long().cuda()
                oriidx = torch.arange(key_contexts.size()[1] - 1).long().cuda()
                prev_x = key_contexts.new(*key_contexts.size()).fill_(0)
                prev_x[:, dstidx] = torch.index_select(key_contexts, 1, oriidx)  # [b x ts x d]
                prev_x = torch.cumsum(prev_x, 1)  # debugged, sums on correct dim per batch
                prev_x[:, 0, :] = 1
                step_x = key_contexts.new(key_contexts.size()[0], key_contexts.size()[1], 1).fill_(0)
            ...
            if step is not None:
                if step>0: #test after second step
                    prev_key_contexts[i] += key_contexts
                else:
                    if prev_key_contexts is None:
                        prev_key_contexts = []
                    prev_key_contexts.append(key_contexts)
