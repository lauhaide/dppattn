...
class Trainer(object):
    ...
    def _forward(self, sample, eval=False):
        ...
        logging_output = {
            ...
            'ntexts': sample['target'].size(0) if sample is not None else 0,
        }
        oom = 0
        if sample is not None:
            try:
                ...
                if not eval and 'out of memory' in str(e):
                    print('| WARNING: ran out of memory, skipping batch *forward {} / tgt {}'.format(sample['ntokens'], sample['target'].shape))
                    oom = 1
                    loss = None
                else:
                    raise e
        ...
        if loss is not None:
            try:
                ...
                if 'out of memory' in str(e):
                    ...
                    print('| WARNING: ran out of memory, skipping batch *backward')
                    oom = 1
                    self.zero_grad()
                else:
                    raise e
    ...
    def setModelEpoch(self, epoch):
        self.model.setModelEpoch(epoch)

    def setModelStartDpp(self, start_epoch):
        self.model.setModelStartDpp(start_epoch)