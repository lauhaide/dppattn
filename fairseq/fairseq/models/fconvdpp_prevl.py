...
@register_model('fconvdpp_prevl')
class FConvModelDPP(FairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        return FConvModelDPP(encoder, decoder)

    def setModelEpoch(self, epoch):
        self.currepoch = epoch


@register_model('fconvdpp_prevl_lm')
...
class FConvEncoder(FairseqEncoder):
    ...
    def __init__(
        ...
        #print("encoder left padding: {}".format(self.left_pad))
        self.embed_positions = PositionalEmbedding(
            max_positions,
            embed_dim,
            self.padding_idx,
            left_pad=self.left_pad
            #left_pad=False, ##see what to do with this, fix LearnedPositionalEmbedding.forward() for the case of True
        )
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        if src_tokens.gt(50000).sum() >0:
            print("exceed voc idx (voc size {})".format(len(self.dictionary)))
            print(src_tokens)
            exit()

        if src_tokens.lt(1).sum() >0:
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x, prev_x, target_embedding, encoder_out, encoder_padding_mask, incremental_state, ln=0):
        residual = x

        # attention
        x = (self.in_projection(x) + target_embedding) * math.sqrt(self.normalization_constant)

        x = self.bmm(x, encoder_out[0]) # quality measure, best suits current context
        ...
        x = x.view(sz)
        attn_scores = x # attention scores here are the Quality w.r.t to the query

        if ln==0: #first layer use normal attention

            x = self.bmm(x, encoder_out[1]) # [b x ts x d]

        else:
            # Build the previous context to measure dissimilarity too

            if prev_x is None and incremental_state is not None: # test time first step
                prev_x = x.new(*target_embedding.size()).fill_(1)


            # Compute cosine similarity of elements w.r.t previous context

            # get here either in training or inference after first time-step
            #if incremental_state is None or (incremental_state is not None and prev_x is not None):
            prev_x = F.normalize(prev_x, p=2, dim=2)
            enc_out = F.normalize(encoder_out[1], p=2, dim=2).transpose(1,2)
            sim_prev = self.bmm(prev_x, enc_out)

            # DPP
            dpps = 1 - (sim_prev**2)

            if (sim_prev != sim_prev).sum() > 0: #nan
                print("context similarity")
                print(sim_prev)
                print(attn_scores)

            dpps = dpps * attn_scores  # element-wise mult. [b x ts x ss]

            x = self.bmm(dpps, encoder_out[1])  # [b x ts x d]
            attn_scores = dpps #re-weighted attn scores
...
class FConvDecoder(FairseqIncrementalDecoder):
    ...
    def _init(self, dictionary, embed_dim=512, embed_dict=None, out_embed_dim=256,
        ...
        # print("decoder left padding: {}".format(self.left_pad))
        convolutions = extend_conv_spec(convolutions)
        in_channels = convolutions[0][0]
    ...
    def forward(self, prev_output_tokens, encoder_out_dict=None, incremental_state=None, prev_contexts=None):
        ...
        step = prev_output_tokens.size()[1]

        # start of sequence (i.e. first step)?
        sos = prev_output_tokens.eq(self.dictionary.eos()).all().item() == 1
        ...
        for proj, conv, attention, res_layer in zip(self.projections, self.convolutions, self.attention,
            ...
            if attention is not None:
                ...
                if ln > 0 and incremental_state is None: # training
                    dstidx = torch.arange(1, contexts.size()[1]).long().cuda()
                    oriidx = torch.arange(contexts.size()[1] - 1).long().cuda()
                    prev_x = contexts.new(*contexts.size()).fill_(0)
                    prev_x[:, dstidx] = torch.index_select(contexts, 1, oriidx)  # [b x ts x d]
                    prev_x = torch.cumsum(prev_x, 1)  # debugged, sums on correct dim per batch
                    prev_x[:, 0, :] = 1
                    step_x = contexts.new(contexts.size()[0], contexts.size()[1], 1).fill_(0)
                    step_x[:, 1:] = dstidx.unsqueeze(1)
                    step_x[:, 0, :] = 1
                    prev_x = prev_x / step_x  # averaged sum

                elif ln > 0 and incremental_state is not None and not sos: # test after second step
                    prev_x = prev_contexts[ln-1] / (step-1) # recover context from previous layer [b x 1 x d]


                x, attn_scores, contexts = attention(x, prev_x, target_embedding,
                                                     (encoder_a, encoder_b), encoder_padding_mask,
                                                     incremental_state, ln)
                ...
                if incremental_state is not None:
                    if not sos: #test after second step
                        prev_contexts[ln] += contexts
                    else:
                        if prev_contexts is None:
                            prev_contexts = []
                        prev_contexts.append(contexts.new(*contexts.size()).fill_(0))
            # residual
            if residual is not None:
                x = (x + residual) * math.sqrt(self.normalization_constant)
            residuals.append(x)
        ...
        return x, avg_attn_scores, prev_contexts
...
@register_model_architecture('fconvdpp_prevl_lm', 'fconvdpp_prevl_lm')
...
@register_model_architecture('fconvdpp_prevl_lm', 'fconvdpp_prevl_lm_dauphin_wikitext103')
...
@register_model_architecture('fconvdpp_prevl_lm', 'fconvdpp_prevl_lm_dauphin_gbw')
...
@register_model_architecture('fconvdpp_prevl', 'fconvdpp_prevl')
...
@register_model_architecture('fconvdpp_prevl', 'fconvdpp_prevl_wikicatsum')
def fconv_wikicatsum(args):
    ...
    #args.encoder_layers = getattr(args, 'encoder_layers', '[(512, 3)] * 6')
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.decoder_embed_path = getattr(args, 'decoder_embed_path', None)
...
@register_model_architecture('fconvdpp_prevl', 'fconvdpp_prevl_iwslt_de_en')
...
@register_model_architecture('fconvdpp_prevl', 'fconvdpp_prevl_wmt_en_ro')
def fconv_wmt_en_ro(args):
    args.decoder_out_embed_dim = getattr(args, 'decoder_out_embed_dim', 512)
    base_architecture(args)


@register_model_architecture('fconvdpp_prevl', 'fconvdpp_prevl_wmt_en_de')
...
@register_model_architecture('fconvdpp_prevl', 'fconvdpp_prevl_wmt_en_fr')
