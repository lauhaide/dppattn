...
from fairseq.preprocess.MalletLDATopicDistAnnotator import MalletLDATopicDistAnnotator
...
def get_parser():
    ...
    # options relevant to generate WikiCatSum corpus to be read within OpenNMT
    parser.add_argument('--out-idx-valid', default="",
                       help="""Path to file with indices to exlucde from corpus Valid.""")
    parser.add_argument('--out-idx-train', default="",
                       help="""Path to file with indices to exlucde from corpus Train.""")
    parser.add_argument('--outfileTexts', action='store_true', help='Generate corpus for those binarised instances')
...
def main(args):
    ...
    def make_binary_dataset(input_prefix, output_prefix, lang):
        ...
        # if indices to exclude are given (case of generating corpus for OpenNMT)
        excludIndex = []
        f=None
        if hasattr(args, 'out_idx_train') and 'train' in input_file:
            f = open(args.out_idx_train, 'r')
        if hasattr(args, 'out_idx_valid') and 'valid' in input_file:
            f = open(args.out_idx_valid, 'r')
        if f is not None:
            excludIndex = [int(l.strip()) for l in f.readlines()]


        if args.singleSeq:
            if '.src' in input_file:
                res = TokenizerWCSSingleSequence.binarizeRepeat(input_file, dict, consumer, L=args.L,
                                                          aconsumer=aconsumer, annotator=annotator,
                                                          R2Rrerank=True, reptype=None, outfileTexts=args.outfileTexts, #R2Rrerank=True, reptype="E"
                                                          excludIndex=excludIndex)
                TokenizerWCSSingleSequence.printStats(res, lang, input_file, dict)
            else:
                res = TokenizerWCSSingleSequence.binarize(input_file, dict, consumer, #L=300,
                                                          aconsumer=aconsumer, annotator=annotator,
                                                          outfileTexts=args.outfileTexts, excludIndex=excludIndex)
                TokenizerWCSSingleSequence.printStats(res, lang, input_file, dict)
        else:
            if '.src' in input_file:
