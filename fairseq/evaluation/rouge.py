...
def rouge_l_recall(eval_sentences, ref_sentences):
  """Computes ROUGE-L (sentence level) of two collections of sentences.
  ...
  for eval_sentence, ref_sentence in zip(eval_sentences, ref_sentences):
    ...
    n = len(eval_sentence)
    lcs = _len_lcs(eval_sentence, ref_sentence)
    recall.append(_r_lcs(lcs, m, n))
  return np.mean(recall, dtype=np.float32)
...
def rouge_n_recall(eval_sentences, ref_sentences, n=2):
  ...
  # return overlapping_count / reference_count
  return np.mean(recalls, dtype=np.float32)  ##L.P. fix , was returning last value