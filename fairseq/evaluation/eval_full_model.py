...
def doEval(decode_dir, metric, cmdrouge):
    ...
    if metric=='rouge':
        dec_pattern = r'(\d+).dec'
        ref_pattern = '#ID#.ref'
        output = eval_rouge(dec_pattern, dec_dir, ref_pattern, ref_dir, cmd=cmdrouge)
    else:
        dec_pattern = '[0-9]+.dec'
        ref_pattern = '[0-9]+.ref'
...
def main(args):
    ...
    output = doEval(args.decode_dir, metric, args.cmd_rouge)
    print(output)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cmd_rouge', action='store', default='-c 95 -r 1000 -n 2 -m', required=False,
                        help='rouge command line options')
