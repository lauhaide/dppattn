...
if __name__ == "__main__":
    ...
    beam_fashion= ""
    if "--beam" in ARG_LIST[0] and "1" in ARG_LIST[1]: #not very secure, may introduce a redundant parameter??
        beam_fashion= "_b"+ARG_LIST[1]

    ngramb = ""
    if "--ngram" in ARG_LIST:
        f = open(os.path.join(args.model_dir, 'best_rouge_ngramb' + beam_fashion + '.txt'), 'w')
        ngramb = "_ngramb"
    else:
        f = open(os.path.join(args.model_dir, 'best_rouge' + beam_fashion + '.txt'), 'w')
    ...
    for filename in os.listdir('./'+str(args.model_dir)):
        if filename.endswith(".pt") and doFile(filename, args.model_dir):
            ...
            DECODEDIR = DECODEDIR +'_' + filename.split('.pt')[0] + beam_fashion + ngramb
            ...
            curr_avg_rouge = (curr_rouge_1 + curr_rouge_2 + curr_rouge_L) /3
